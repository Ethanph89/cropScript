...
# MAIN
def main():
    print("Are you working with a 'Selects' folder? (Y/N)")
    selects = input().lower()

    if selects == 'y':
        print("Please choose the Selects folder of the rig you'd like to modify:")
        pathToFolder = browse_button()
        pathlist = Path(pathToFolder).glob('**/*.xmp')
    else:
        print("Please choose the _JPG_CROP folder of the rig you'd like to modify:")
        pathToFolder = browse_button()
        pathlist = Path(pathToFolder).glob('**/*.jpg')

    print("Would you like to modify all images, or only selected ones? (type 'all', '2dark', or '2light')")
    userRange = input().lower()
    while userRange != "all" and userRange != "2dark" and userRange != "2light":
        print("Range not recognized. Please re-type.")
        print("Would you like to modify all images, or only selected ones? (type 'all', '2dark', or '2light')")
        userRange = input().lower()
    ...
    for path in pathlist:
        ...
        XMPmod(xmpPath, userL, usera, userb, userRange)
...
# changes XMP vals by user givern percent
def XMPmod(path, uL, ua, ub, urange):
    f_tmp = open(path + '_tmp', 'w')
    selected = False

    # XMP mod for all images
    if urange == "all":
        print("Modifying all images...")
    ...
    # XMP mod for 2dark images
    elif urange == "2dark":
        with open(path, 'r') as f:
            for line in f:
                if "xmp:Label=\"Select\"" in line:
                    selected = True
            f.close()
        ...
        with open(path, 'r') as f:
            if selected:
                ...
                print(str(path) + " is not selected")
                f_tmp.close()
                remove(path + '_tmp')

    # XMP mod for 2light images
    elif urange == "2light":
        with open(path, 'r') as f:
            for line in f:
                if "xmp:Label=\"Second\"" in line:
                    selected = True
            f.close()
